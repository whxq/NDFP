<template>
    <div  style=" position: relative; height: 100%">
        ...
        <div ref="chartRef" style="width: 100%;margin-top: 20px; height: 120px;"></div>


        <transition >
            <div style="width: 100%; height: 240px" v-if="isCompareMode">
                <div class="first-title-text" style="line-height: 35px;">
                    基准数据
                </div>
                <div ref="chartRefStd"  style="width: 100%; margin-top:10px;height: 105px;"></div>
<!--                结果表-->
                <div v-if="isCompareMode" class="table-container">
                    <el-table :data="resData" stripe >
                        <el-table-column fixed="left" width="100" align="center">
                            <template #default="scope">
                                <el-row justify="end" style="padding-left: 0px">
                                    <div style="font-weight: bold!important; font-size: 17px;">
                                        {{val[scope.$index]}}
                                    </div>
                                </el-row>
                            </template>
                        ...
                        <el-table-column
                            v-for="(column, columnIndex) in resData[1]"
                            align="center"
        ...
        <transition>
            <el-row v-if="!isCompareMode">
                <div class="title-container">
                    <div class="first-title-text" style="line-height: 35px; margin-bottom: 7px; margin-top: 35px">
                        异常流量具体信息(部分)
                    </div>
                </div>
                <div class="title-container" style="margin-left: 390px">
                    <div class="first-title-text" style="line-height: 35px; margin-bottom: 7px; margin-top: 35px">
                        数据面板
                    </div>
                </div>
        ...
        <div style="position: absolute; bottom: -5.4%;">
            <el-container style="margin-top: 5px; transition: height 0.7s ease"
                          :style="{height:(isCompareMode)?'325px':'595px'}">
                <el-aside style="width: 752px; background-color: white" class="table-container" >
                    <el-table
                        ref="myTable"
                ...
                <el-main class="panel-container" :style="{height:(isCompareMode)?'325px':'595px'}"
                         style="transition: height 0.7s ease">
                    <el-scrollbar>
                        <div ref="innerRef" style="width: 100%; margin-top: 10px"></div>
                        <div ref="innerRef2" style="width: 100%; margin-top: 20px"></div>
                    </el-scrollbar>
...
let nameArray = ["xss", "DDoS", "password", "scanning", "injection", "DoS", "Backdoor", "mitm", "ransomware"]
...
function iniSeriesData() {
    let total = Object.values(maxValues).reduce((accumulator, currentValue) => {
    ...
    seriesDataStd.sort((a, b) => b.data[0] - a.data[0])
}
...
function updateMainGraph(additionalDataItem){
    if (!isComplete){
        ...
        Object.keys(additionalDataItem).forEach((key)=>{
            currentValues[key] += additionalDataItem[key]
...
function showStdGraph(){
    resData.value[0] = Object.values(currentValues)
    resData.value[1] = Object.values(maxValues)
    resData.value[2] = resData.value[0].map((value, index) => {
        if (resData.value[0][index] == 0) {
            return '未识别出'
        } else
        return (value *100/ resData.value[1][index]).toFixed(2) + '%';
    })
...
watch(isCompareMode, (newVal) => {
    tableHeight.value = newVal ? '300px' : '570px';
});
