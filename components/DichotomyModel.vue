<template>
            ...
            <div>
                <el-dropdown placement="bottom-start" class="custom-dropdown">
                    <div class="first-title-text" style="margin-bottom: 5px">
                        双阈值收敛过程(10轮学习): {{curTitle + ' Samples'}}
                    </div>
                    <template #dropdown>
                        <el-dropdown-menu class="custom-dropdown-menu">
                            ...
                            <el-dropdown-item :class="[activeNum == 2 ? 'choice-item-selected' : 'choice-item']" @click="setTitle('CIC-IDS-2018',2)">CIC-IDS-2018 Samples</el-dropdown-item>
    ...
    <div ref="chartRef" style="width: 100%; margin-top: 15px"></div>
...
let preTimer = null
let notification
const goodPointScale = 11
const attackPointScale = 12
...
function setTitle(newTitle, newActiveNum){
    ...
    TRAINING_DATA.forEach((item) => {
        if (item.name === newTitle) {
            preTimer = readLine(item.data, newTitle)
        }
    })
}
...
//读取一行数据
function readLine(tmpData, newTitle){
    notification?.close()
    data.length = 0
    let i = 0
    data.push(tmpData[0])
    ...
    let timer = setInterval(() => {
        if (tmpData[i] != null) {
            ...
            notification = ElNotification({
                ...
                message: '<strong>'+ newTitle +' 10轮训练完成！</strong>',
                type: 'success',
            })
            clearInterval(timer)
        }
...
function iniFirstData(){
    attackFigureHeight = 10
    goodFigureHeight = 10
    goodThresholds[0].value = 10
    attackThresholds[0].value = 10
    option = {
        // 指定图表的类型为柱状图
        type: 'bar',
        // 指定图表的标题
        ...
        legend: {
            ...
            data: [
                { name: '攻击阈值', icon: 'roundRect', itemStyle: { color: 'rgba(238, 102, 102, 0.8)' } },
                { name: '良性阈值', icon: 'roundRect', itemStyle: { color: 'rgba(145, 204, 117, 0.8)' }  },
...
let colorStops = [{
    offset: 0,
    color: 'rgba(238, 102, 102, 0.02)'  // 渐变起始颜色
}, {
    offset: 1,
    color: 'rgba(238, 102, 102, 0.6)'    // 渐变结束颜色
}];

let colorStops2= [{
    offset: 0,
    color: 'rgba(145, 204, 117, 0.6)'  // 渐变起始颜色
}, {
    offset: 1,
    color: 'rgba(145, 204, 117, 0.02)'    // 渐变结束颜色
}];
...
onMounted(() => {
    ...
    //             iniFirstData()
    //         }
    //         console.log('attackFigureHeight', attackFigureHeight)
    //
    //     }
